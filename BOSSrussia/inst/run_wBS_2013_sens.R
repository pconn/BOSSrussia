...
X_bd = model.matrix(~Strata*(ice_conc+ice2+dist_mainland+dist_shelf),X_s)
X_rd = model.matrix(~Strata*(ice_conc+ice2+dist_mainland+dist_shelf),X_s)
...
#minus strata
X_tmp = model.matrix(~ice_conc+ice2+dist_mainland+dist_shelf,X_s)
Sens_mods[[3]][[9]] = as.matrix(X_tmp[,-1])
Sens_mods[[4]][[13]] = as.matrix(X_tmp[,-1])
#minus mainland
X_tmp = model.matrix(~Strata*(ice_conc+ice2+dist_shelf),X_s)
Sens_mods[[3]][[10]] = as.matrix(X_tmp[,-1])
Sens_mods[[4]][[14]] = as.matrix(X_tmp[,-1])
#minus shelf
X_tmp = model.matrix(~Strata*(ice_conc+ice2+dist_mainland),X_s)
Sens_mods[[3]][[11]] = as.matrix(X_tmp[,-1])
Sens_mods[[4]][[15]] = as.matrix(X_tmp[,-1])
#add contour
X_tmp = model.matrix(~Strata*(ice_conc+ice2+dist_shelf+dist_mainland+dist_contour),X_s)
...
Params_orig=Params
  ...
  for(isens in 1:n_sens){  #note haven't modified Beta_VCs because not using priors in analysis
    ...
    if(Opt$convergence==0 & Opt$message!="X_convergence (3)"){
      ...
      save.image('sens_out_wBS2013.RData')
    }
  }
  ...
  Opt = nlminb( start=Obj$par, objective=Obj$fn, gradient=Obj$gr, lower=Lower, upper=Upper, control=list(trace=1, eval.max = 10000,maxit=10000))         #
  Opt[["diagnostics"]] = data.frame( "Param"=names(Obj$par), "Lower"=-Inf, "Est"=Opt$par, "Upper"=Inf, "gradient"=Obj$gr(Opt$par) )
  Report = Report_noMisID=Obj$report() 
