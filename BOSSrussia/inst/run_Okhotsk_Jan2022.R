...
Sens_table = data.frame("model"=c("base","no misID","rd_Bering","-depth","-dist_land","-dist_break","-dist_edge"),
                        "LogL" = c(-mod_base$jnll,-mod_no_misID$jnll,-LogL[6],-LogL[-c(2,6,7)]),
                        "k" = 4*c(8,8,8,6,7,7,7),
                        "AIC" = rep(0,7),
                        "N_bd" = c(mod_base$total_abundance[3],mod_no_misID$total_abundance[3],N[3,6],N[3,-c(2,6,7)]),
                        "N_rn" = c(mod_base$total_abundance[2],mod_no_misID$total_abundance[2],N[2,6],N[2,-c(2,6,7)]),
                        "N_rd" = c(mod_base$total_abundance[4],mod_no_misID$total_abundance[4],N[4,6],N[4,-c(2,6,7)]),
                        "N_sd" = c(mod_base$total_abundance[1],mod_no_misID$total_abundance[1],N[1,6],N[1,-c(2,6,7)])
                        )
Sens_table$AIC = -2*Sens_table$LogL + 2*Sens_table$k
Sens_table$AIC[2:7]=Sens_table$AIC[1]-Sens_table$AIC[2:7]
Sens_table$AIC[1]=0
library(xtable)
print(xtable(Sens_table,digits=c(0,0,1,0,1,0,0,0,0)),include.rownames=FALSE)
...
Sens_table = data.frame("model"=c("base","no misID","rd_Bering","-depth","-dist_land","-dist_break","-dist_edge"),
                        "n_bd" = Extrap_num[3,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "n_rn" = Extrap_num[2,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "n_rd" = Extrap_num[4,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "n_sd" = Extrap_num[1,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "Lambda_bd" = Extrap_ratio[3,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "Lambda_rn" = Extrap_ratio[2,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "Lambda_rd" = Extrap_ratio[4,c(1,2,7,3:6)], #just switching model order, leaving out jday availability
                        "Lambda_sd" = Extrap_ratio[1,c(1,2,7,3:6)] #just switching model order, leaving out jday availability
)
library(xtable)
